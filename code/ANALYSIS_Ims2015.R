...
### CREATE TABLE
sink(paste0("results/IMS2015_table1_",data.name,".csv"))
for (myvar in model.name) {
...
if( paste0(best$formula)[3] == "Treatment * Position + (1 | Channel)"){
  ...
  TreatmentGHB <- mcmc[,1:CORES,"TreatmentGH:PositionB"]
  TreatmentGHC <- mcmc[,1:CORES,"TreatmentGH:PositionC"]
  TreatmentGHD <- mcmc[,1:CORES,"TreatmentGH:PositionD"]
  TreatmentGHE <- mcmc[,1:CORES,"TreatmentGH:PositionE"]
  ...
  tmp <-  paste0(
    ...
    , best$formula
    , ","
    , round(quantile(TreatmentGHA , probs=c(.5), na.rm=TRUE),2)
    , " [", round(quantile(TreatmentGHA , probs=c(.025), na.rm=TRUE),2), "; ", round(quantile(TreatmentGHA , probs=c(.975), na.rm=TRUE),2),"]"
    , ", "
    , round(mean(TreatmentGHA>0),3) # P: confidence that the parameter is positive (P>0)
  )
  print(tmp[3])
  ...
  tmp <-  paste0(
    ...
    , best$formula
    , ","
    , round(quantile(TreatmentGHB , probs=c(.5), na.rm=TRUE),2)
    , " [", round(quantile(TreatmentGHB , probs=c(.025), na.rm=TRUE),2), "; ", round(quantile(TreatmentGHB , probs=c(.975), na.rm=TRUE),2),"]"
    , ", "
    , round(mean(TreatmentGHB>0),3) # P: confidence that the parameter is positive (P>0)
  )
  print(tmp[3])
  ...
  tmp <-  paste0(
    ...
    , " [", round(quantile(TreatmentGHC , probs=c(.025), na.rm=TRUE),2), "; ", round(quantile(TreatmentGHC , probs=c(.975), na.rm=TRUE),2),"]"
    , ", "
    , round(mean(TreatmentGHC>0),3) # P: confidence that the parameter is positive (P>0)
  )
  print(tmp[3])
  ...
  tmp <-  paste0(
    ...
    , " [", round(quantile(TreatmentGHD , probs=c(.025), na.rm=TRUE),2), "; ", round(quantile(TreatmentGHD , probs=c(.975), na.rm=TRUE),2),"]"
    , ", "
    , round(mean(TreatmentGHD>0),3) # P: confidence that the parameter is positive (P>0)
  )
  print(tmp[3])
  ...
  tmp <-  paste0(
    ...
    , " [", round(quantile(TreatmentGHE , probs=c(.025), na.rm=TRUE),2), "; ", round(quantile(TreatmentGHE , probs=c(.975), na.rm=TRUE),2),"]"
    , ", "
    , round(mean(TreatmentGHE>0),3) # P: confidence that the parameter is positive (P>0)
  )
  print(tmp[3])
} # end if
