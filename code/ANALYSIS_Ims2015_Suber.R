...
# Load dataset:
library(readxl)
Data_SURBER_2015 <- read_excel("~/Documents/RESEARCH/PROJECTS/Biodiversa/SalmoInvade/Ims/data/
                               ")
#View(Data_SURBER_2015)

# Cange format of data:
data <- as.data.frame(Data_SURBER_2015)
...
### SAVE
save(best,loo, file=paste0("results/IMS2015_Surber_",model.name,"_",data.name,".Rdata"))
}
...
### SAVE ALL
#save(Total_Prim_Cons, Total_Sec_Cons,Pred_Prey_Ratio,Rhyacophilidae,Chironomidae,Polycentropodidae,Hydropsychidae,Baetidae,Simuliidae,Planorbidae, file=paste0("results/IMS2015_Surber_",data.name,".Rdata"))
...
sink(paste0("results/IMS2015_table3_",data.name,".csv"))
for (myvar in model.name) {
  
  ## LOAD MODEL
  best = loo = mcmc = NULL
  load(paste0("results/IMS2015_Surber_",myvar,"_",data.name,".Rdata"))
  ...
  if( paste0(best$formula)[3] == "Treatment * Position + (1 | Channel)"){
    ...
    tmp <-  paste0(
      ...
      , best$formula
      , ","
      , round(quantile(TreatmentGHD , probs=c(.5), na.rm=TRUE),3)
      , " [", round(quantile(TreatmentGHD , probs=c(.025), na.rm=TRUE),3), "; ", round(quantile(TreatmentGHD , probs=c(.975), na.rm=TRUE),3),"]"
      , ", "
...
#   gap <- .05
#   plot(NULL,xlim=c(1,4),ylim=c(min(y.pred),max(y.pred)),ylab=ylab,xlab=xlab,xaxt="n",main=main,bty="n")
...
#   legend("topright",legend=c("SHAM", "GH"),col=mycol[1:2],pch=c(16,16),bty="n")
...
#   gap <- .05
#   plot(NULL,xlim=c(0,4),ylim=range(model$stan_summary[3:10,c("2.5%","97.5%")]),ylab=ylab,xlab=xlab,xaxt="n",main=main,bty="n")
#   axis(1, labels=LETTERS[2:5], at = 1:3)
#   abline(h=0,lty=2)
...
# 
# ## SUMMARY
# sink(paste0("results/IMS2015_Surber_summary_",data.name,".txt"))
# for (myvar in model.name) {
#   
#   ## LOAD MODEL
#   best = loo = mcmc = NULL
...
#   } else {
#     
#     q <- quantile(TreatmentGHA, probs=c(.025, .25, .5, .75, .975),2) # quantiles
#     p <- gf(TreatmentGHA)
#     res <- c(q,P=p)
#   }
#     print(res)
...
# for (myvar in model.name) {
#   #myvar= "Sum_SRP"
#   pdf(paste0("results/IMS2015_Surber_",myvar,"_check_",data.name,".pdf"),width=8, height = 8)
#   
#   ## LOAD MODEL
#   best = loo = mcmc = NULL
#   load(paste0("results/IMS2015_Surber_",myvar,"_",data.name,".Rdata"))
...
# 
# ## FIGURES EFFETCS
# pdf(paste("results/IMS2015_Surber_",data.name,".pdf",sep=""),width=8, height=10,)
# par(mfrow=c(3,2))
...
#   points(as.numeric(as.factor(best$data$Position)), best$y, col=mycol[as.numeric(as.factor(best$data$Treatment))], pch=as.numeric(as.factor(best$data$Channel)))
# }
# 
# } # end loop myvar
# dev.off()
